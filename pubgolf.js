(async function () {
    ...
    let items = []; // Array om de nummers van locaties op te slaan
    ...
    // Laad locaties wanneer de pagina geladen is
    window.addEventListener('DOMContentLoaded', init);

    async function init(firstInit = true, groups = 1, duration = 1, resetMode = false) {
        // Haal de locatienummers op uit pubgolf.json en voeg deze toe aan de items array
        const data = await fetch('pubgolf.json').then(response => response.json());
